...
- [ ] **B2.** Seed migration `0003_seed_oll_cases.sql` — idempotent insert of 3×3 puzzle type, OLL stage, all 57 cases (port from `initial_design/src/data.jsx`), result-case backfill pass. Integration test asserts 57 cases post-migrate.

### Backend endpoints (paired with frontend below)
- [ ] **C1.** `cases::merge` helper module + `GET /cases` (override-merged list). Unit test for the COALESCE merge across each overridable field. Depends on B1, B2.
- [ ] **C2.** `GET /cases/:id` — same merge for a single row. 404 on unknown ID. Depends on C1.
- [ ] **C3.** `PATCH /cases/:id/settings` — upsert into `user_case_settings`, validate `result_case_id` belongs to the same stage, delete row when all nulls. Returns merged `Case`. Depends on C1.

### Frontend
- [ ] **D1.** App shell + tab bar. Promote `HomeView` into `AppShell.vue` with three tabs; Practice and Progress are placeholder views (`PracticeStubView`, `ProgressStubView`). Top-right user icon routes to `/settings`. Cases tab is wired but its target is the next ticket. Depends on M1.
- [ ] **D2.** `PatternDiagram.vue` — port from `initial_design/src/diagram.jsx`. Storybook-free; render the seven dot-group patterns in a small dev sandbox view as a visual sanity check. Pattern-rotation helper `frontend/src/lib/pattern.ts` with Vitest tests. Depends on D1.
- [ ] **D3.** `casesStore` Pinia store. Fetches via `api.get('/cases')`, exposes `byId` / `groupedByTier2`, `updateSettings()` action that PATCHes and replaces the row in place. Depends on C1.
- [ ] **D4.** `CasesView.vue` — header, search, Tier 1 chips, grouped grid of `CaseTile`. Wires `casesStore.ensureLoaded()`. Depends on D2, D3.
- [ ] **D5.** `CaseDetailView.vue` — pattern + algorithm + result-preview cards, edit mode swapping fields for inputs, Save calls `casesStore.updateSettings()`. Depends on D2, D3, C2, C3.
